...
extension [T] (self: ValidatedNel[BrokenConfinement, T]) {
  def reportedAs(key: String): ValidatedNel[BrokenConfinement, T] = { self match
  ...
  def -> (key: String): ValidatedNel[BrokenConfinement, T] =
    self.reportedAs(key)
}

extension [Actual] (self: Actual) {
  inline def validateAs[P1]: ValidatedConfinement[Actual & P1] =
    macros.ConfinementAccumulator.accumulate[Actual, P1].confine(self)

  inline def validateIs[P1](using IV: Checker[Actual, P1]): Boolean =
...
extension [A, Out](fn: (A) => Out) {
  transparent inline def validating[F[_]](fa: CF[A]) =
    fa.map(fn)
}
  
extension [A, B, Out](fn: (A, B) => Out) {
  transparent inline def validating(fa: CF[A], fb: CF[B]): (F[Out]) =
    Semigroupal.map2[F, A, B, Out](fa, fb)(fn)
}

extension [A, B, C, Out](fn: (A, B, C) => Out) {
  transparent inline def validating(fa: CF[A], fb: CF[B], fc: CF[C]): (F[Out]) =
    Semigroupal.map3[F, A, B, C, Out](fa, fb, fc)(fn)
}

extension [A, B, C, D, Out](fn: (A, B, C, D) => Out) {
  transparent inline def validating(fa: CF[A], fb: CF[B], fc: CF[C], fd: CF[D]): (F[Out]) =
    Semigroupal.map4(fa, fb, fc, fd)(fn)
}

extension [A, B, C, D, E, Out](fn: (A, B, C, D, E) => Out) {
  transparent inline def validating(fa: CF[A], fb: CF[B], fc: CF[C], fd: CF[D], fe: CF[E]): (F[Out]) =
    Semigroupal.map5(fa, fb, fc, fd, fe)(fn)
}

extension [A, B, C, D, E, G, Out](fn: (A, B, C, D, E, G) => Out) {
  transparent inline def validating(fa: CF[A], fb: CF[B], fc: CF[C], fd: CF[D], fe: CF[E], fg: CF[G]): (F[Out]) =
